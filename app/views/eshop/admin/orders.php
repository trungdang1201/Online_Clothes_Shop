...
<table class="table table-striped table-advance table-hover">
	...
	<thead>
		<tr><th>Order no</th><th>Customer</th><th>Order date</th><th>Total</th><th>Delivery Address</th><th>Mobile Phone</th><th>Status</th><th>...</th></tr>
	</thead>
	<tbody onclick="show_details(event)">
		...
		<?php foreach($orders as $order):?>
			...
			<tr style="position: relative;"><td><?=$order->id?></td><td><a href="<?=ROOT?>profile/<?=$order->user->url_address?>"><?=$order->user->name?></a></td><td><?=date("jS M Y H:i a",strtotime($order->date))?></td><td>$<?=$order->total?></td><td><?=$order->delivery_address?></td><td><?=$order->phone?></td>
				...
				<td>
					...
					<div class="js-order-details details hide" >
						...
						<div style="display: flex;">
							...
							<table class="table" style="flex: 1;margin: 4px;">
								<tr><th>Mobile Phone</th><td><?=$order->phone?></td></tr>
							...
							<?php if(isset($order->details) && is_array($order->details)):?>
								<tbody>
									<?php foreach($order->details as $detail):?>
										<tr><td><?=$detail->qty?></td><td><?=$detail->amount?></td><td><?=$detail->total?></td></tr>
